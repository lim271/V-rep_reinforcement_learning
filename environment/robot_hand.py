...
class Robot_hand(Core):
    ...
    def launch(self):
        ...
        self.brick_handle=vrep.simxGetObjectHandle(self.clientID, \
            'Brick_red',vrep.simx_opmode_blocking)[1]
        self.epoch=0
        self.count=0
    ...
    def reset(self):
        ...
        vrep.simxSetObjectPosition(self.clientID, \
            self.brick_handle,-1,self.goal+[0.22],vrep.simx_opmode_blocking)
        self.state0=None
        self.action_prev=[0.0,0.0]
        self.goal_dist_prev=None
    ...
    def step(self,action,return_obs=False):
        ...
        while vrep.simxGetLastCmdTime(self.clientID)-t<self.dt:
            ...
            goal_pos=vrep.simxGetObjectPosition(self.clientID, \
                self.brick_handle,self.body_handle,vrep.simx_opmode_streaming)[1]
            joint_pose=[ \
                vrep.simxGetJointPosition( \
                    self.clientID,self.joint_handles[idx],vrep.simx_opmode_streaming)[1] \
                    for idx in range(6)]
